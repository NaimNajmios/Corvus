...
private val Context.geminiDataStore: DataStore<Preferences> by preferencesDataStore(name = "gemini_quota")

@Singleton
class GeminiQuotaGuard @Inject constructor(
    @ApplicationContext private val context: Context
) {
    companion object {
        const val DAILY_LIMIT = 1500
        const val MONTHLY_LIMIT = 30000

        private val KEY_CALLS_TODAY = intPreferencesKey("gemini_calls_today")
        private val KEY_RESET_DATE = longPreferencesKey("gemini_quota_reset_date")
        private val KEY_MONTHLY_CALLS = intPreferencesKey("gemini_monthly_calls")
        private val KEY_MONTHLY_RESET = longPreferencesKey("gemini_monthly_reset")
    }
    ...
    suspend fun canCall(): Boolean {
        ...
        val prefs = context.geminiDataStore.data.first()
    ...
    suspend fun monthlyCallsUsed(): Int =
        context.geminiDataStore.data.first()[KEY_MONTHLY_CALLS] ?: 0
