...
private val Context.openrouterDataStore: DataStore<Preferences> by preferencesDataStore(name = "openrouter_quota")

@Singleton
class OpenRouterQuotaGuard @Inject constructor(
    @ApplicationContext private val context: Context
) {
    companion object {
        const val DAILY_LIMIT = 200
        const val MONTHLY_LIMIT = 5000

        private val KEY_CALLS_TODAY = intPreferencesKey("openrouter_calls_today")
        private val KEY_RESET_DATE = longPreferencesKey("openrouter_quota_reset_date")
        private val KEY_MONTHLY_CALLS = intPreferencesKey("openrouter_monthly_calls")
        private val KEY_MONTHLY_RESET = longPreferencesKey("openrouter_monthly_reset")
    }
    ...
    suspend fun monthlyCallsUsed(): Int =
        context.openrouterDataStore.data.first()[KEY_MONTHLY_CALLS] ?: 0
